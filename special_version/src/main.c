...
/*   Updated: 2025/05/29 15:23:32 by tdaniel-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	validate_game_map(t_game *game)
{
	...
	{
		ft_printf("The map must be a rectangle.\n");
		ft_quit_game(game);
	}
	if (is_firstlast_wall_ok(game) == false || is_side_walls_ok(game) == false)
	{
		ft_printf("Map borders must be surrounded by walls.\n");
		ft_quit_game(game);
	}
	if (are_there_collectibles(game) == false)
	{
		ft_printf("The map doesnt have collectibles.\n");
		ft_quit_game(game);
	}
	if (is_there_exit_and_player(game) == false)
	{
		ft_printf("There isnt a player or an exit.\n");
		ft_quit_game(game);
	}
	if (is_there_only_allowed_chars(game) == false)
	{
		ft_printf("Characters allowed [C,P,E,1,0] only \n");
		ft_quit_game(game);
	}
}
